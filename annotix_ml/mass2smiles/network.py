...
class Mass2SmilesModel(nn.Module):
    def __init__(
        ...
        self.encoder_layers = nn.ModuleList(
            [
                EncoderLayer(
                    d_model=embed_dim, num_heads=heads, dff=units, dropout_rate=dropout
                )
...
def prepro_specs_train(df):
    max_len_mz = max(
        [
            len(mzs) + len(loss) + 1
            for _, (mzs, loss) in df[["mzs", "loss_mzs"]].iterrows()
        ]
    )
    max_len_intensities = max(
        [
            len(intensities) + len(loss) + 1
            for _, (intensities, loss) in df[
                ["intensities", "loss_intensities"]
            ].iterrows()
        ]
    )
    assert max_len_mz == max_len_intensities, (
        "Max length of mz and intensities should be the same"
    )
    logger.info(f"Max length of mz/intensities: {max_len_mz}")

    valid = []
    for _, (precursor_mz, mzs, intensities, loss_mzs, loss_intensities) in df[
        ["precursor_mz", "mzs", "intensities", "loss_mzs", "loss_intensities"]
    ].iterrows():
        mz_list = [
            round(float(trun_n_d(str(precursor_mz), 2)) * 100)
        ]  # add precursor mz
        intes_list = [
            2.0
        ]  # add precursor intensity, arbitrary value > 1 to distinguish from real peaks
    ...
    return torch.nn.utils.rnn.pad_sequence(
        [torch.tensor(v, dtype=torch.float32) for v in valid], batch_first=True
    )
    ...
    for sample in rag_tensor:
        ...
        if len(valid_intensities) == 0:
            ...
            normalized_intensities = (
                valid_intensities / max(valid_intensities)
                if max(valid_intensities) > 0
                else valid_intensities
            )

            # Stack m/z and intensity as features
            peaks = torch.stack(
                [
                    torch.tensor(normalized_mz, dtype=torch.float32),
                    torch.tensor(normalized_intensities, dtype=torch.float32),
                ],
                dim=1,
            )
...
class SpectrumEncoder(nn.Module):
    ...
    def forward(self, mz, intensities):
        ...
        mz_scaled = (
            (mz / self.mz_max * self.num_mz_bins).clamp(0, self.num_mz_bins - 1).long()
        )
    ...
    def encode(self, smiles, max_length=None):
        ...
        if max_length is not None:
            if len(indices) > max_length:
                ...
                indices.extend(
                    [self.token_to_idx["<PAD>"]] * (max_length - len(indices))
                )
    ...
    def __getitem__(self, idx):
        ...
        if self.model_type == "direct":
            ...
            spectrum_input = np.stack(
                [mz_padded / 1000.0, int_padded], axis=1
            )  # Scale m/z
            spectrum_tensor = torch.tensor(spectrum_input, dtype=torch.float32)
        else:  # learned
            # For learned encoding: separate tensors
            spectrum_tensor = (
                torch.tensor(mz_padded, dtype=torch.float32),
                torch.tensor(int_padded, dtype=torch.float32),
            )

        # Encode SMILES
        smiles_encoded = self.tokenizer.encode(
            row["smiles"], max_length=self.max_smiles_length
        )
...
def prepare_data_loaders(
    ...
    # Create datasets
    train_dataset = SpectrumSMILESDataset(
        train_path, tokenizer, max_spectrum_length, max_smiles_length, model_type
    )
    val_dataset = SpectrumSMILESDataset(
        val_path, tokenizer, max_spectrum_length, max_smiles_length, model_type
    )
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, (spectrum, smiles_target) in enumerate(train_pbar):
            ...
            try:
                ...
                target_first_token = smiles_target[
                    :, 1
                ]  # Skip START token, predict first real token
...
class SMILESDecoder(nn.Module):
    ...
    def forward(
        self, memory, tgt_tokens=None, tgt_mask=None, memory_key_padding_mask=None
    ):
        """
    ...
    def forward(self, spectrum, smiles_target=None, teacher_forcing_ratio=1.0):
        ...
        if (
            ...
            smiles_logits = self.smiles_decoder(
                memory=memory, tgt_tokens=decoder_input, tgt_mask=tgt_mask
            )
        else:
            ...
            if smiles_target is not None:
                ...
                smiles_logits = torch.zeros(
                    batch_size,
                    target_len - 1,
                    self.smiles_decoder.vocab_size,
...
def combined_loss(
    ...
    smiles_loss = F.cross_entropy(
        smiles_logits_flat, smiles_target_flat, ignore_index=ignore_index
    )
...
def prepare_seq2seq_data_loaders(
    csv_path,
    batch_size=16,
    test_size=0.2,
    max_spectrum_length=500,
    ...
    def seq2seq_collate_fn(batch):
        if model_type == "direct":
            ...
            return (
                (torch.stack(mz_tensors), torch.stack(int_tensors)),
                torch.stack(smiles),
                torch.stack(fgs),
            )
    ...
    val_loader = DataLoader(
        val_dataset, batch_size=batch_size, shuffle=False, collate_fn=seq2seq_collate_fn
    )

    logger.info(
        f"Created seq2seq data loaders - Train: {len(train_dataset)}, Val: {len(val_dataset)}"
    )

    return train_loader, val_loader, tokenizer


def train_seq2seq_mass2smiles(
    model,
    train_loader,
    val_loader,
    num_epochs=10,
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, (spectrum, smiles_target, fg_target) in enumerate(train_pbar):
            ...
            try:
                # Forward pass
                if model_type == "direct":
                    smiles_logits, fg_pred = model(
                        spectrum, smiles_target, teacher_forcing_ratio
                    )
                else:  # learned
                    smiles_logits, fg_pred = model(
                        spectrum[0], spectrum[1], smiles_target, teacher_forcing_ratio
                    )
        ...
        avg_train_loss = train_loss / train_batches if train_batches > 0 else 0
        avg_train_smiles_loss = (
            train_smiles_loss / train_batches if train_batches > 0 else 0
        )
        ...
        with torch.no_grad():
            ...
            for batch_idx, (spectrum, smiles_target, fg_target) in enumerate(val_pbar):
                try:
                    ...
                    if model_type == "direct":
                        smiles_logits, fg_pred = model(
                            spectrum, smiles_target, teacher_forcing_ratio=1.0
                        )
                    else:  # learned
                        smiles_logits, fg_pred = model(
                            spectrum[0],
        ...
        logger.info(
            f"  Train - Total: {avg_train_loss:.4f}, SMILES: {avg_train_smiles_loss:.4f}, FG: {avg_train_fg_loss:.4f}"
        )
        logger.info(
            f"  Val   - Total: {avg_val_loss:.4f}, SMILES: {avg_val_smiles_loss:.4f}, FG: {avg_val_fg_loss:.4f}"
        )

        # Save checkpoint
        if save_path and (epoch + 1) % 5 == 0:
            checkpoint_path = (
                Path(save_path) / f"seq2seq_checkpoint_epoch_{epoch + 1}.pth"
            )
            checkpoint_path.parent.mkdir(parents=True, exist_ok=True)
            torch.save(
                {
