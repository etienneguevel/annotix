...
from annotix_ml.spectraencoder.data.abstract_dataset import (
...
from annotix_ml.spectraencoder.data.featurizers import (
    PairedFeaturizer,
    GraphFeaturizer,
...
from annotix_ml.spectraencoder.data.datasets import (
    get_paired_loader_graph,
    SpectraMolDataset,
    get_paired_spectra,
)
from annotix_ml.spectraencoder.data.splitter import PresetSpectraSplitter
from annotix_ml.spectraencoder.utils import to_dense
...
class Spec2MolDataModule(MolecularDataModule):
    def __init__(self, cfg):
        self.remove_h = False
        self.datadir = BASE_DIR / cfg.dataset.datadir
        self.filter_dataset = cfg.dataset.filter
        self.train_smiles = []

        data_splitter = PresetSpectraSplitter(
            split_file=BASE_DIR / cfg.dataset.split_file
        )

        paired_featurizer = PairedFeaturizer(
            spec_featurizer=PeakFormula(**cfg.dataset),
            mol_featurizer=FingerprintFeaturizer(
                fp_names=["morgan4096"],
                **cfg.dataset,
                includeChirality=False,
            ),
            graph_featurizer=GraphFeaturizer(**cfg.dataset),
        )

        spectra_mol_pairs = get_paired_spectra(**cfg.dataset)
    ...
    def valency_count(self, max_n_nodes):
        ...
        for batch in self.train_dataloader():
            ...
            for atom in range(n):
                ...
                v = (edges_total * multiplier).sum()
                valencies[v.long().item()] += 1
...
class Spec2MolDatasetInfos(AbstractDatasetInfos):
    def __init__(self, datamodule, cfg, recompute_statistics=False, meta=None):
        ...
        self._root_path = BASE_DIR / cfg.dataset.datadir
