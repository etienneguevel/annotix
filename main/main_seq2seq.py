...
def main():
    ...
    parser.add_argument(
        "--csv_path",
        type=str,
        default="main/test_data/test-data.csv",
    ...
    parser.add_argument(
        "--max_length", type=int, default=150, help="Maximum SMILES sequence length"
    )
    parser.add_argument(
        "--test_generation",
        action="store_true",
    ...
    parser.add_argument(
        "--device", default="cpu", help="Device to use: 'cpu' or 'cuda' or 'mps'"
    )
    ...
    if args.model == "direct":
        ...
        print(
            summary(
                model,
                input_data=(input_spectra.unsqueeze(0), target_tokens.unsqueeze(0)),
        ...
        print(
            summary(
                model,
                input_data=(
                    input_mz.unsqueeze(0),
                    input_intensities.unsqueeze(0),
    ...
    if args.model == "direct":
        ...
        logger.info(
            f"Input m/z shape: {mz_values.shape}, intensities shape: {intensities.shape}"
        )
    ...
    with torch.no_grad():
        if args.model == "direct":
            ...
            smiles_logits, fg_pred = model(
                mz_values, intensities, target_tokens[:, :-1]
            )
    ...
    if args.test_generation:
        ...
        if args.model == "direct":
            ...
            generated_tokens = trained_model.generate(
                input_spectra, tokenizer, max_length=args.max_length, temperature=0.8
            )
        else:
        ...
        with torch.no_grad():
            if args.model == "direct":
                _, fg_pred = trained_model(
                    input_spectra, target_tokens[:-1].unsqueeze(0)
                )
            else:
                _, fg_pred = trained_model(
                    input_mz, input_intensities, target_tokens[:-1].unsqueeze(0)
                )
            ...
            logger.info(
                f"Predicted FG: {predicted_fg.sum().item():.0f} functional groups"
            )
