...
def main():
    ...
    parser.add_argument(
        "-input", required=True, help="Path to CSV file with spectrum and SMILES data"
    )
    parser.add_argument(
        "-model_type",
    ...
    parser.add_argument(
        "-save_path",
        default="/tmp/mass2smiles_models",
    ...
    train_loader, val_loader, tokenizer = prepare_data_loaders(
        csv_path=args.input, batch_size=args.batch_size, model_type=args.model_type
    )
    ...
    with torch.no_grad():
        for spectrum, smiles_target in val_loader:
            ...
            target_token_idx = smiles_target[
                0, 1
            ].item()  # First real token (skip START)

            pred_token = tokenizer.idx_to_token.get(pred_token_idx, "<UNK>")
            target_token = tokenizer.idx_to_token.get(target_token_idx, "<UNK>")

            logger.info(
                f'Sample prediction - Target: "{target_token}", Predicted: "{pred_token}"'
            )
